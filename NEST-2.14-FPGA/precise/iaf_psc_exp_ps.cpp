...
RecordablesMap< iaf_psc_exp_ps >::create()
{
  ...
  insert_( names::weighted_spikes_ex, &iaf_psc_exp_ps::get_weighted_spikes_ex_ );
  insert_( names::weighted_spikes_in, &iaf_psc_exp_ps::get_weighted_spikes_in_ );
  insert_( names::I_syn_ex, &iaf_psc_exp_ps::get_I_syn_ex_ );
  insert_( names::I_syn_in, &iaf_psc_exp_ps::get_I_syn_in_ );
}
}
...
nest::iaf_psc_exp_ps::calibrate()
{
  ...
  // a double in ms. The grid based iaf_psc_exp_ps can only handle refractory
...
nest::iaf_psc_exp_ps::update( const Time& origin, const long from, const long to )
{
  ...
  //int signal_;
  int thrd = nest::kernel().vp_manager.get_thread_id();
  int flag = nest::kernel().simulation_manager.get_flag();
  int num = nest::kernel().simulation_manager.get_ONETIME_num();
  int num0 = nest::kernel().simulation_manager.get_num_n(thrd);
  
  if(flag == 0)
  {
    if(num0 == 0)
    {
      nest::kernel().simulation_manager.set_stable(thrd, V_.P20_, V_.P22_, V_.P21ex_,
      V_.P21in_, V_.P11ex_, V_.P11in_, P_.Theta_, V_.RefractoryCounts_,
      P_.V_reset_);
    }
    nest::kernel().simulation_manager.set_in(thrd, num0, S_.i_syn_ex_, S_.i_syn_in_,
    S_.r_ref_, S_.V_m_);
    nest::kernel().simulation_manager.set_tt(thrd, num0, P_.I_e_);
    nest::kernel().simulation_manager.set_variable(thrd, num0, 0, 0);
  }
  else if(flag == 1)
  {
    nest::kernel().simulation_manager.set_tt(thrd, num0, P_.I_e_);
    nest::kernel().simulation_manager.set_stable_1(thrd, P_.Theta_, V_.RefractoryCounts_,
    P_.V_reset_);
	
    V_.weighted_spikes_ex_ = B_.spikes_ex_.get_value( from );
    V_.weighted_spikes_in_ = B_.spikes_in_.get_value( from );
    nest::kernel().simulation_manager.set_variable(thrd, num0, V_.weighted_spikes_ex_,
    V_.weighted_spikes_in_);
  }
  /*
  else if(flag == 2)
  {
    ...
    nest::kernel().simulation_manager.set_variable(num, V_.weighted_spikes_ex_,
    V_.weighted_spikes_in_);
  }
  */
  else if(flag == 3)
  {
    //nest::kernel().simulation_manager.return_data(num, signal_);

    //S_.i_syn_ex_ = i_syn_ex;
    //S_.i_syn_in_ = i_syn_in;
    //S_.r_ref_ = r_ref;
    //S_.V_m_ = V_m;
	
    //if (signal_ == 1) // threshold crossing
    //{
    ...
    nest::kernel().simulation_manager.set_variable(thrd, num, V_.weighted_spikes_ex_,
    V_.weighted_spikes_in_);
      //B_.logger_.record_data( origin.get_steps() + from );
    //}
	/*
    else
    {
...
nest::iaf_psc_exp_ps::handle( SpikeEvent& e )
{
  ...
  if ( e.get_weight() >= 0.0 )
  {
	...
	//kernel().simulation_manager.handle_weight_ex(num, (e.get_weight() * e.get_multiplicity()) );
  }
  else
  {
    B_.spikes_in_.add_value( e.get_rel_delivery_steps(
                               kernel().simulation_manager.get_slice_origin() ),
      e.get_weight() * e.get_multiplicity() );
	//kernel().simulation_manager.handle_weight_in(num, (e.get_weight() * e.get_multiplicity()) );
  }
}
